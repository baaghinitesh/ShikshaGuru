...
export const searchTeachers = async (req: Request, res: Response) => {
  try {
    const {
      ...
      experience,
      minRating = 0,
      maxDistance = 25000, // Default 25km
      latitude,
      longitude,
      minBudget,
      maxBudget,
    ...
    let distanceBuckets: any[] = [];
    if (latitude && longitude) {
      distanceBuckets = DISTANCE_BUCKETS.map(bucket => ({
        ...bucket,
        count: teachers.filter((teacher: any) => 
          teacher.distance >= bucket.min && teacher.distance < bucket.max
        ).length
      }));
    }
...
export const searchJobs = async (req: Request, res: Response) => {
  try {
    ...
    if (latitude && longitude) {
      distanceBuckets = DISTANCE_BUCKETS.map(bucket => ({
        ...bucket,
        count: jobs.filter((job: any) => 
          job.distance >= bucket.min && job.distance < bucket.max
        ).length
      }));
    }
...
export const getNearbyResults = async (req: Request, res: Response) => {
  try {
    const { latitude, longitude, maxDistance = 10000, type = 'teachers' } = req.query as any;
    ...
    if (type === 'teachers') {
      ...
      results = await Teacher.aggregate([
        geoNearStage,
        { $limit: 50 },
        {
          $project: {
...
export const getLocationSuggestions = async (req: Request, res: Response) => {
  try {
    ...
    const teacherLocations = await Teacher.aggregate([
      {
        $match: {
          $or: [
    ...
    const uniqueLocations = allLocations.reduce((acc: any[], curr) => {
      const existing = acc.find(item => 
        item._id.city === curr._id.city && item._id.state === curr._id.state
      );
      if (!existing) {
        acc.push(curr);
      } else {
